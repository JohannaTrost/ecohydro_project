...
dir<-"C:/Users/vince/OneDrive/Desktop/Ecohydrology/mixing model - water uptake"

#date preprocessing
filename <- file.path(dir,"/2026 Example Data.csv")
...
mix <- data[1:54, ]
write.csv(mix, file.path(dir, "mix_data.csv"), row.names = FALSE)

source <- data[55:99, ]
write.csv(source, file.path(dir, "source_data.csv"), row.names = FALSE)


# To run on your data, replace the system.file call with the path to your file
mix.filename <- file.path(dir,"/mix_data.csv")
# Load mixture data
mix <- load_mix_data(filename=mix.filename, 
                     iso_names=c("X2H","X18O"), 
                     factors=c("Block","Type"), 
...
# To run on your data, replace the system.file call with the path to your file
source.filename <- file.path(dir,"/source_data.csv")

# Load source data
source <- load_source_data(filename=source.filename, source_factors="Depth", 
                            conc_dep=FALSE, data_type="raw", mix)
